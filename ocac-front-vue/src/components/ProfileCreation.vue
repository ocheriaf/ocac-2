<template>
  <div class="bg-green-100 m-6 rounded shadow p-2">
    <h1 class="text-green-900 text-2xl font-bold">Profile Creation</h1>
    <p class="text-green-900 text-sm ">
      Your profile is incomplete. You can complete it by following the next
      steps.
    </p>
    <div class="  px-1 py-3 rounded mt-6">
      <div class="flex xl:w-4/5 xl:mx-auto 2xl:w-3/5 ">
        <div
          class="w-1/3 text-center px-6 opacity-75"
          :class="{ 'opacity-100': step === 1 }"
        >
          <div
            class="bg-green-600 rounded-lg flex items-center justify-center border-none border-green-200 transform hover:scale-105"
          >
            <div
              class="w-1/3 bg-transparent h-10 flex items-center justify-center icon-step "
            >
              <svg
            ...
            <div
              class="w-2/3 bg-green-900 h-10 flex flex-col items-center justify-center px-1 rounded-r-lg body-step"
            >
              <h2 class=" text-sm text-green-100">Personal Info</h2>
            </div>
          </div>
        </div>

        <div
          class="w-1/3 text-center px-3 opacity-75"
          :class="{ 'opacity-100': step === 2 }"
        >
          <div
            class="bg-green-500 rounded-lg flex items-center justify-center border-none border-green-200 transform hover:scale-105"
          >
            <div
              class="w-1/3 bg-transparent h-10 flex items-center justify-center icon-step"
            >
            ...
            <div
              class="w-2/3 bg-green-900 h-10 flex flex-col items-center justify-center px-1 rounded-r-lg body-step"
            >
              <h2 class=" text-sm text-green-100">Interest</h2>
            </div>
          </div>
        </div>
        ...
        >
          ...
          >
            ...
            <div
              class="w-2/3 bg-green-900 h-10 flex flex-col items-center justify-center px-1 rounded-r-lg body-step"
            >
              <h2 class="text-sm text-green-100">Confirmation</h2>
            </div>
          </div>
        </div>
      </div>
      <div class="mt-3">
        <div v-if="step === 1">TODO</div>
        <div v-if="step === 2">TODO</div>
        <div v-if="step === 3">TODO</div>
      </div>

      <button
        @click="nextStep"
        class=" mt-3 bg-green-700 px-3 py-2 text-green-100 rounded  focus:border-green-900 focus:ring focus:outline-none focus:ring-green-600 focus:ring-opacity-50"
      >
        Continue
      </button>
    </div>
